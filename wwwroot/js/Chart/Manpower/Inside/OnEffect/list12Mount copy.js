...
var month = d.getMonth() + 1;
    ...
    var tartgetDetails = [];
    $.get(origin + "/Home/GetListMonthInfoDetailByDate/" + getMount,
        function (data, textStatus, jqXHR) {
            $.each(data, function (indexInArray, valueOfElement) {
                var tartgetDetail = [];
                tartgetDetail.push(valueOfElement.company_Code);
                tartgetDetail.push(valueOfElement.target);
                tartgetDetail.push(valueOfElement.target_adds);
                tartgetDetail.push(valueOfElement.info);
                tartgetDetail.push(valueOfElement.adds);
...
var htmlListmonth = 'A summary of the template options that are available to you.<p />' +
    ...
    var graph = ['เป้าหมาย', 'พนักงานทั้งหมด', 'อัตราว่าง'];
